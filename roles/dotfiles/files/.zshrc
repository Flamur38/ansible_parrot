...
case "$TERM" in
xterm*|rxvt*)
    TERM_TITLE=']0;${debian_chroot:+($debian_chroot)}%n@%m: %~'
    ;;
*)
    ;;
esac
...
if [ -x /usr/bin/dircolors ]; then
    ...
    export LESS_TERMCAP_mb=$'[1;31m'     # begin blink
    export LESS_TERMCAP_md=$'[1;36m'     # begin bold
    export LESS_TERMCAP_me=$'[0m'        # reset bold/blink
    export LESS_TERMCAP_so=$'[01;33m'    # begin reverse video
    export LESS_TERMCAP_se=$'[0m'        # reset reverse video
    export LESS_TERMCAP_us=$'[1;32m'     # begin underline
    export LESS_TERMCAP_ue=$'[0m'        # reset underline
